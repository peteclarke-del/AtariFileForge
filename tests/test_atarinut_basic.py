...
def kinds(line) -> list[tuple]:
    """Every token on one line, as its kind and the text it covers."""
...
class RecognitionTests(unittest.TestCase):
    def test_detect_names_the_dialect_from_the_bytes(self):
        cases = [
        ...
        for name, dialect, verdict in cases:
            with self.subTest(name=name):
                detection = detect(fixture(name))
                self.assertIs(
                    detection.dialect, dialect,
                    f"{name} was read as {detection.dialect}: {detection.reason}",
                )
                self.assertIs(detection.verdict, verdict)
                self.assertTrue(detection.reason)


    def test_detect_refuses_what_is_not_basic(self):
    ...
    def test_detect_says_why_a_related_file_is_not_a_program(self):
    ...
    def test_trailing_bytes_are_reported_rather_than_swallowed(self):
    ...
    def test_is_tokenised_is_about_storage_not_about_being_basic(self):
...
class GfaBasic3Tests(unittest.TestCase):
    def test_gfa_listing_round_trips_through_the_tokeniser(self):
        source = listing("tilemap.lst")
        assert detokenise(tokenise(source, GFA_BASIC_3)) == source


    def test_gfa_program_round_trips_through_the_lister(self):
        program = fixture("tilemap.gfa")
        assert tokenise(detokenise(program), GFA_BASIC_3) == program
        assert gfa.canonicalise(program) == program


    def test_a_real_gfa_listing_reaches_a_stable_form(self):
    ...
    def test_gfa_indentation_comes_from_block_structure(self):
    ...
    def test_gfa_floats_survive_the_eight_byte_encoding(self):
        for value in (0.0, 1.0, -1.0, 0.5, 3.5, 1e10, -2.25e-8, 3.14159265358979):
            self.assertAlmostEqual(
                gfa.gfa_double_to_float(gfa.float_to_gfa_double(value)), value,
                delta=abs(value) * 1e-9 + 1e-12,
            )
        assert gfa.float_to_gfa_double(0.0) == bytes(8)


    def test_a_truncated_gfa_file_is_refused_with_its_reason(self):
        program = fixture("tilemap.gfa")
...
class StosBasicTests(unittest.TestCase):
    def test_stos_program_lists_as_its_source(self):
        assert stos.detokenise_stos(fixture("stars.bas")) == listing("stars.asc")
        assert detokenise(fixture("stars.bas")) == listing("stars.asc")


    def test_stos_program_round_trips_through_the_corpus_writer(self):
    ...
    def test_stos_refuses_to_be_written_and_says_why(self):
    ...
    def test_stos_literals_decode_to_their_written_values(self):
    ...
    def test_stos_fast_floating_point_survives_a_round_trip(self):
        for value in (0.0, 1.0, -1.0, 2.5, 0.125, 1000.0, -0.0625):
            self.assertAlmostEqual(
                stos.ffp_to_float(stos.float_to_ffp(value)), value,
                delta=abs(value) * 1e-6 + 1e-9,
            )
        assert stos.float_to_ffp(0.0) == bytes(4)
        assert stos.ffp_to_float(bytes.fromhex("c8000046")) == 50.0


    def test_an_unrecoverable_stos_token_lists_as_a_placeholder(self):
    ...
    def test_a_malformed_stos_line_is_refused(self):
...
class StBasicTests(unittest.TestCase):
    def test_st_basic_is_stored_as_the_text_it_lists_as(self):
    ...
    def test_st_basic_is_told_apart_from_a_stos_listing(self):
...
class ScanningTests(unittest.TestCase):
    def test_scanning_types_every_element_of_a_line(self):
    ...
    def test_a_rem_takes_the_rest_of_its_line_as_a_comment(self):
    ...
    def test_a_typed_name_that_resembles_a_command_stays_a_variable(self):
    ...
    def test_scanning_a_gfa_program_offers_spans_into_the_file(self):
        program = fixture("tilemap.gfa")
    ...
    def test_scanning_a_stos_program_offers_spans_into_the_file(self):
        program = fixture("stars.bas")
        for line in scan_program(program):
            for token in line.tokens:
    ...
    def test_scanning_never_needs_the_whole_listing_first(self):
...
class DialectMetadataTests(unittest.TestCase):
    def test_dialect_metadata_is_coherent(self):
    ...
    def test_gfa_2_is_a_listing_dialect_without_the_3x_words(self):
...
class DecodeProgramTests(unittest.TestCase):
    def test_decode_program_reports_the_dialect_and_whether_it_can_be_saved(self):
        cases = [
            ("tilemap.gfa", "gfa-basic-3", True),
            ("stars.bas", "stos-basic", False),
            ("clock.bas", "st-basic", True),
        ]
        for name, dialect_id, writable in cases:
            with self.subTest(name=name):
                decoded = decode_program(fixture(name))
                self.assertIsNotNone(decoded)
                self.assertEqual(decoded.dialect_id, dialect_id)
                self.assertIs(decoded.writable, writable)
                self.assertTrue(decoded.lines)
                self.assertTrue(all(line.text for line in decoded.lines))


    def test_decode_program_returns_nothing_for_anything_else(self):
