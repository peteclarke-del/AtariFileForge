"""DiskMasher (DMS) archives: a whole Atari floppy, compressed track by track.

DMS was how Atari disks travelled. Unlike an ADF, which is just the sectors,
...
#: Eighty of them make the 880 KiB an GEMDOS double-density disk holds.
TRACK_SIZE = 2 * 11 * 512
BLOCK_SIZE = 512
...
DISK_TYPES = {
    ...
    1: "GEMDOS OFS",
    2: "GEMDOS FFS",
    3: "GEMDOS International OFS",
    4: "GEMDOS International FFS",
    5: "GEMDOS Directory Cache OFS",
    6: "GEMDOS Directory Cache FFS",
    7: "Professional File System",
    8: "MS-DOS",
    9: "Claude/Atari custom",
}
...
#: release wrote it.
DMS_PROJECT_SCHEMA = "atari-file-forge/dms-project/v1"
...
def is_tokenized_basic(data: bytes) -> bool:
    """True when these bytes are a tokenised ST BASIC program."""
    try:
        from atarinut.basic import is_tokenised
...
def basic_unopened_channel_io(data: bytes) -> bool:
    ...
    """
    try:
        from atarinut.basic import STBASIC_12, detokenise
    except ImportError:  # pragma: no cover - defensive
        return False
    if not is_tokenized_basic(data):
        return False
    try:
        listing = detokenise(data, dialect=STBASIC_12).upper()
    except Exception:
        return False
...
def rewrite_basic_loader(
    ...
    """
    try:
        from atarinut.basic import STBASIC_12, detokenise, tokenise
    except ImportError:  # pragma: no cover - defensive
        return data, []
    if not is_tokenized_basic(data):
        return data, []
    try:
        listing = detokenise(data, dialect=STBASIC_12)
    except Exception:
        return data, []
